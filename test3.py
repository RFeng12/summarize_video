...
def download_images(keywords, num, index, paragraph, outputslice, im_size):
    ...
    for i in range(0, 10):
        time.sleep(0.5)
        try:
            unfiltered_urls = bing_image_urls(keywords.strip(), limit=num*3)
        except Exception:
            continue
    ...
    if len(url_list) > 1:
        if outputslice == True:
            url_list = url_list[:int(len(url_list)/2)]
        else:
            url_list = url_list[int(len(url_list)/2):]
    ...
    if len(url_list) == 0 or keywords == "noimagefound":
        for i in range(0, num):
            ...
            write_source(image_path, paragraph, 20, im_size[1]-100, 100, font)
